...
class Interpreter implements Expr.Visitor<Object>, Stmt.Visitor<Object> {
    ...
    public Object visitCallExpr(Expr.Call expr) {
        ...
        for (Expr argumentExpr : expr.args) {
    ...
    public Object visitBlockStmt(Stmt.Block stmt) {
        ...
        for (Stmt s : stmt.statements) {
            if (s instanceof Stmt.Function) {
                Stmt.Function funStmt = (Stmt.Function) s;
                SimplfFunction function = new SimplfFunction(funStmt, environment);
                blockEnv.define(funStmt.name, funStmt.name.lexeme, function);
            }
        }
    
        for (Stmt s : stmt.statements) {
            if (s instanceof Stmt.Var) {
                Stmt.Var varStmt = (Stmt.Var) s;
                blockEnv.define(varStmt.name, varStmt.name.lexeme, null);
            }
        }
        ...
        try {
            for (Stmt s : stmt.statements) {
                // Skip already-hoisted functions
                if (s instanceof Stmt.Function) continue;
                execute(s);
            }
        } finally {
