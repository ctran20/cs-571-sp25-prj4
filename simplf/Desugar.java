...
public class Desugar implements Expr.Visitor<Expr>, Stmt.Visitor<Stmt> {
    ...
    public Stmt visitForStmt(Stmt.For stmt) {
        Stmt initStmt = new Stmt.Expression(stmt.init);
        Expr condExpr = stmt.cond;
        Expr incrExpr = stmt.incr;

        List<Stmt> bodyStmts = new ArrayList<>();
        bodyStmts.add(stmt.body);
        bodyStmts.add(new Stmt.Expression(incrExpr));
        Stmt whileLoop = new Stmt.While(condExpr, new Stmt.Block(bodyStmts));

        List<Stmt> outerBlock = new ArrayList<>();
        outerBlock.add(initStmt);
        outerBlock.add(whileLoop);

        return new Stmt.Block(outerBlock);
    }
